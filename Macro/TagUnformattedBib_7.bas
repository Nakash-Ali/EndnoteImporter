...
Sub TagUnformattedBib()
    'TagItalic
    ReplaceRepeatedAuthor
    TagRecordBegin
    TagRecordRefType
End Sub

Sub ReplaceRepeatedAuthor()
    ...
    For i = 1 To ActiveDocument.Paragraphs.Count
        ...
        If Len(Selection.Text) > 1 Then
            ...
            Do While continue
                If ((Selection.Next = ".") And (Selection.Next(Count:=2) = " ") And _
                ((Selection.Next(Count:=3) = ChrW(8216)) Or (Selection.Next(Count:=3) = ChrW(34)) Or (Selection.Next(Count:=3) = ChrW(8220)) Or (Selection.Next(Count:=3) = "<"))) Then
                    currentAuthor = Selection.Text
                    If currentAuthor = "___" And paValid Then
                        Selection.Text = previousAuthor
                    Else
                    ...
                    continue = False
                End If
                If (Selection.Next = Chr(10)) Or (Selection.Next = Chr(13)) Then
                    continue = False
                    paValid = False
                End If
                Selection.MoveEnd Unit:=wdCharacter, Count:=1
            Loop
            Debug.Print previousAuthor
            If Not paValid Then
                Debug.Print "invalid"
            End If
        End If
...
Sub TagRecordRefType()
    ...
    ' Define TD ref type for thesis/dissertations
    Dim refTypeTD As ReferenceType
    refTypeTD.Tag = "DT:TD."
    Set refTypeTD.Patterns = New Collection
    ...
    ' Regex patterns for Thesis/Dissertation:
    ' -------------------------------
    refTypeTD.Patterns.Add "x--*. " & "[" & ChrW(8216) & ChrW(34) & ChrW(8220) & "]" & "*" & "[" & ChrW(8217) & ChrW(34) & ChrW(8221) & "]" & "* thesis, *[Uu]nivers*.*"
    refTypeTD.Patterns.Add "x--*. " & "[" & ChrW(8216) & ChrW(34) & ChrW(8220) & "]" & "*" & "[" & ChrW(8217) & ChrW(34) & ChrW(8221) & "]" & "* thesis, *[Ii]nstit*.*"
    refTypeTD.Patterns.Add "x--*. " & "[" & ChrW(8216) & ChrW(34) & ChrW(8220) & "]" & "*" & "[" & ChrW(8217) & ChrW(34) & ChrW(8221) & "]" & "* diss., *[Uu]nivers*.*"
    refTypeTD.Patterns.Add "x--*. " & "[" & ChrW(8216) & ChrW(34) & ChrW(8220) & "]" & "*" & "[" & ChrW(8217) & ChrW(34) & ChrW(8221) & "]" & "* diss., *[Ii]nstit*.*"
    
    ' Define reference type collection
    Dim refTypes(1 To 5) As ReferenceType
    refTypes(1) = refTypeBook
    refTypes(2) = refTypeJournal
    refTypes(3) = refTypeMS
    refTypes(4) = refTypeBookSec
    refTypes(5) = refTypeTD
